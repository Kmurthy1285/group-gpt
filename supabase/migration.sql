-- Migration script to update existing database for authentication
-- Run this instead of the full schema.sql if you have existing data

-- Enable UUID ext (if not already enabled)
...
add column if not exists name text default 'Untitled Chat',
add column if not exists created_by uuid references public.user_profiles(id) on delete set null,
add column if not exists updated_at timestamptz default now();

-- Update existing rooms to have a default name
update public.rooms 
set name = 'Chat Room ' || substring(id::text, 1, 8)
where name is null or name = 'Untitled Chat';
...
drop policy if exists "insert rooms" on public.rooms;
